...
def run_stock_research(query: str):
    """Run the stock research agent and return the full message content with debug logging."""
    try:
        ...
        if not messages:
            ...
            logging.debug(
                f"[run_stock_research] Output content from dict: {output_text}"
            )
        elif hasattr(messages[-1], "content"):
            output_text = messages[-1].content
            logging.debug(
                f"[run_stock_research] Output content from object: {output_text}"
            )
        else:
        ...
        if "files" in result:
            ...
            for filename, content in result["files"].items():
                ...
                file_output += f"\n**{filename}**\n{preview}\n"
                logging.debug(
                    f"[run_stock_research] File: {filename}, Preview: {preview[:100]}"
                )

        # Return the full output (no streaming to avoid chunk overwrite in the UI)
        full_output = output_text + file_output
        return full_output
...
custom_css = """
        /* Increase the markdown container's min-height and enable scrolling for overflow */
        .gr-markdown {
        min-height: 500px;
        }
        .gr-markdown > div {
...
with gr.Blocks(css=custom_css) as demo:
    ...
    gr.Markdown(
        "Enter your stock research request below. Example: *Comprehensive analysis on Apple Inc. (AAPL)*"
    )
    ...
    # Output (below input)
    research_output = gr.Markdown(label="Research Output")

    # Wire up action
    run_button.click(fn=run_stock_research, inputs=query_input, outputs=research_output)
